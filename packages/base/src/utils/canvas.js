// 背景动画
/**
 * @param {documentElement|string} element
 * */
export function canvas_back(element) {
  const canvas = document.querySelector(element);
  const ctx = canvas.getContext('2d');
  const w = window.innerWidth; // 获取屏幕宽高
  const h = window.innerHeight * 2 / 2;
  canvas.width = w; // 将屏幕宽高赋值给canvas
  canvas.height = h;
  // canvas.style.cssText += `;width:${w}px;height:${h}px;`;
  const dots = [];
  const minScale = 1; // 设置圆点的最小缩放
  const maxScale = 3; // 设置圆点的最大缩放
  // const bei = 1;
  ...
  function Dot() {
    ...
    this.rx = Math.random() * w * (Math.random() > 0.6 ? 2 / 3 : 1 /
      4); // 选一个随机数，作为椭圆长轴顶点a的数值,也就是椭圆中心的x轴坐标，此处的x,y,是相对于椭圆圆心为中心点建立的直角坐标系。
    ...
    this.y = Math.sin(this.d) * this.ry + h /
      2; // 根据椭圆x,y求值公式，x=a*cosα，y=a*sinα.求出圆点在椭圆上的x,y轴坐标.后面加w/2,h/2只是为了居中显示。此处的x,y是相对于canvas建立的直角坐标系的，按canvas约定，其中心点（0,0）点在左上角
    this.z = (Math.pow(Math.pow(this.rx, 2) + Math.pow(this.ry, 2), 0.5) / Math.pow(Math.pow(w / 2, 2) + Math.pow(h /
      2, 2), 0.5)) // 求白色圆点离椭圆中心点的距离，可视为一个点到圆心的距离，用x平方+y平方=c平方求得。此处的x,y,是相对于椭圆圆心为中心点建立的直角坐标系。此处Z值只是一个比值
    this.scale = (Math.cos(this.d) + 1) * (maxScale - minScale) / 2 +
      minScale; // 圆点的缩放大小,根据余弦函数公式y=cosα.标准的余弦函数最大值和最小值分别为1，-1.这里要自行设置最大值和最小值，所以要进行改写
